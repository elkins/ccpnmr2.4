"""
Benchmark random number generation implementations.

Compares performance and statistical quality across:
1. Pure Python LCG (random.py)
2. Numba JIT LCG (random_numba.py)
3. Python stdlib random (Mersenne Twister)
4. NumPy random (PCG64)

Tests both single value and array generation for uniform and normal distributions.
...
# Import stdlib random FIRST before modifying path
...
def test_statistical_quality():
    ...
    for name, gen_func, seed_func in [
        ...
        ('Stdlib (MT)', lambda: _stdlib_random.random(), lambda s: _stdlib_random.seed(s)),
    ...
    for name, gen_func, seed_func in [
        ...
        ('Stdlib (MT)', lambda: _stdlib_random.gauss(0, 1), lambda s: _stdlib_random.seed(s)),
